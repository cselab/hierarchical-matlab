...
function [sys_para] = User_Input_unif_theta_post(index)
...
sys_para.save_file = [ data_folder 'HB_unif_theta_post_' sprintf('%03d',index)];
...
sys_para.lik.name = 'posterior_theta_unif';

file_name = [ data_folder 'data_set_' sprintf('%03d',index) '.mat'];
sys_para.lik.para = load(file_name);

tmp = load([data_folder 'aux_'  sprintf('%03d',index) '.mat']);
sys_para.lik.para.denom = tmp.denom;
sys_para.lik.para.Ns_th = length(tmp.denom);

tmp = load([ data_folder 'IND_theta_' sprintf('%03d',index) '.mat']);
sys_para.lik.para.lnEv  = tmp.out_master.lnEv;


tmp = load([ data_folder 'HB_unif_psi.mat']);
sys_para.lik.para.psi = tmp.out_master.theta;
sys_para.lik.para.Ns_psi = size(tmp.out_master.theta,1);
...
sys_para.pri.name{1} = 'Uniform'; sys_para.pri.para{1} = [250,350];
sys_para.pri.name{2} = 'Uniform'; sys_para.pri.para{2} = [0,100];
sys_para.pri.name{3} = 'Uniform'; sys_para.pri.para{3} = [-5,5];
sys_para.pri.name{4} = 'Uniform'; sys_para.pri.para{4} = [0, 50];

% for i = 1:sys_para.N_dim
%     sys_para.pri.name{i} = 'Uniform';
%     sys_para.pri.para{i} = [-2, 2];
% end
