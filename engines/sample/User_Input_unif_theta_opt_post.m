...
function [sys_para] = User_Input_unif_theta_opt_post(k)
...
sys_para.save_file = [data_folder 'opt_unif_theta_post_' sprintf('%03d',k) ];
...
sys_para.hard_bds = [-400, -400, -400,   0 ; ...
                      400,  400,  400, 100]; 
...
tmp=load([data_folder 'opt_psi.mat']);
psi = tmp.bestever.x;
clear tmp;

sys_para.pri.name{1} = 'Uniform'; sys_para.pri.para{1} = [psi(1),psi(2)];
sys_para.pri.name{2} = 'Uniform'; sys_para.pri.para{2} = [psi(3),psi(4)];
sys_para.pri.name{3} = 'Uniform'; sys_para.pri.para{3} = [psi(5),psi(6)];
sys_para.pri.name{4} = 'Uniform'; sys_para.pri.para{4} = [psi(7),psi(8)];
